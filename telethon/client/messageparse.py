        ...
        try:
            if isinstance(result, types.UpdateShort):
                ...
                self._log[__name__].warning(
                    'Result is not of expected type (UpdateShort, Updates, UpdatesCombined): %s', result)
                return None
            ...
            for update in updates:
                if isinstance(update, types.UpdateMessageID):
                    ...
                    if hasattr(request, 'random_id'):
                        id_to_message[update.message.id] = update.message
                    elif hasattr(request, 'id') and request.id == update.message.id:
                        return update.message

                elif (isinstance(update, types.UpdateEditChannelMessage)
                      and utils.get_peer_id(request.peer) ==
                      utils.get_peer_id(update.message.peer_id)):
                    if hasattr(request, 'id') and request.id == update.message.id:
                        update.message._finish_init(self, entities, input_chat)
                        return update.message
            ...
            self._log[__name__].exception(
                'Error in _get_response_message: %s', e)
